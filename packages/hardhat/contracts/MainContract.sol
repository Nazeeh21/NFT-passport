...
contract MainContract {
  ...
  mapping(address => StampsCollected[]) public userToStampsCollected;
  ...
  function collectStamp(string memory countryName) public {
    require(userToPassportId[msg.sender] > 0, "This address does not own any passport");
    ...
    uint256 stampTokenId = passport.collectStamp(passportTokenId, countryStampAddresses[countryName], msg.sender);
    userToStamp[msg.sender][countryName] = stampTokenId;
    StampsCollected[] storage stampCollected = userToStampsCollected[msg.sender];
    stampCollected.push(StampsCollected({countryName: countryName, stampTokenId: stampTokenId}));
    userToStampsCollected[msg.sender] = stampCollected;
  }
  ...
  function createCountryStamp(
    ...
    CountryStamp countryStamp = new CountryStamp(countryName, currency, dataUri);
    countryStampAddresses[countryName] = address(countryStamp);
    return address(countryStamp);
  }

  function getStampsCollectedByAddress(address user) public view returns (StampsCollected[] memory) {
    return userToStampsCollected[user];
}
